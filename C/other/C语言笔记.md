...
### 移位操作符
**移位和按位中的'位'是指二进制位**
**左移操作符（<<）**将一个运算对象的二进制数值（补码）向左移动指定位数（正整数），通过丢弃高位字节并用低位字节（0）填充，从而为运算对象创造一个左移位
...
**右移操作符（>>）**将一个运算对象的二进制数值（补码）向右移动指定位数，通过丢弃高位字节并用低位字节（0）填充，分为算术右移和逻辑右移
算术右移：和符号相同的填充
逻辑右移：高位填0
一般进行的是算术右移

- **左移** (`<<`): 将一个数的所有位向左移动指定的位数，右边空出的位用0填充。相当于乘以\(2^n\)
- **右移** (`>>`): 将一个数的所有位向右移动指定的位数，对于无符号数，左边空出的位用0填充；对于有符号数，行为依赖于实现，通常左边会填充符号位。相当于除以\(2^n\)
...
- **按位取反** (`~`): 对一个数的每一位进行逻辑非操作。将1变为0，将0变为1。常用于生成一个数的补码
